...
- Exclude pads:
  - (`deleted_pins`) {[`pad number`]}. The pads at the the given positions will be deleted. The pin numbers will remain consecutive: for example if pin 2 is hidden, the numbers will be 1, 2, 3, etc. The package will be named, e.g. `QFN-16-12`.
